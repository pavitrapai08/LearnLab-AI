...
export default function TeacherQuizPage() {
  ...
  const handleQuizComplete = useCallback(async (s: number, total: number, answers: AnswerRecord[]) => {
    ...
    if (quiz?.id) {
      ...
      if (user) {
        await supabase.from('quiz_attempts').insert({ session_id: user.id, quiz_id: quiz.id, score: s, total, answers })
      }
    }
    setStep('results')
  }, [quiz])

  const reset = () => { setStep('upload'); setDocumentId(null); setQuiz(null); setScore(null); setError('') }
        ...
        <div className="mx-auto max-w-2xl">
          ...
          {step === 'upload' && (
            <section className="flex flex-col gap-4">
              <p className="text-sm text-muted-foreground">Upload teaching material to generate a quiz from it.</p>
          ...
          {step === 'configure' && (
            <section className="flex flex-col gap-5">
              ...
              <SubjectLanguageBar subject={subject} grade={grade} outputLanguage={outputLanguage}
          ...
          {step === 'quiz' && quiz && (
            <section className="flex flex-col gap-4">
              ...
              <QuizCard quizId={quiz.id} questions={quiz.questions} questionType={quiz.questionType}
                grade={grade || 'Grade 10'} onComplete={handleQuizComplete} />
            </section>
          )}
          ...
          {step === 'results' && score && (
            <section className="flex flex-col items-center gap-4 py-8">
              <div className="rounded-2xl border bg-card p-8 text-center shadow-sm">
                ...
                <p className="mt-1 text-muted-foreground">{Math.round((score.score / score.total) * 100)}%</p>
                <AiDisclaimer />
              </div>
