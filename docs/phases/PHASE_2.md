...
- [ ] Upload PDF → flashcards extracted correctly; flip works on mobile tap — **[YOU]**.
- [ ] Self-ratings persist; focus mode shows only weak cards — **[YOU]**.
- [ ] Summary renders quick summary + key points + terms, correctly structured — **[YOU]**.
- [ ] Both respect the output-language selector — **[YOU]**.

## QA checks

- [ ] **[YOU]** *Persistence:* cards/ratings survive a reload on the same device (Supabase, not in-memory).
- [ ] **[YOU]** *Anonymous baseline:* opening the app in a different browser shows a fresh session; copy reads "saved to the cloud — here when you return on this device", not "all your devices."
